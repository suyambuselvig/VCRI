app.controller("myCntrl", function ($scope, angularService) {
 //   $scope.divEmployee = false;
    $scope.getformula = function (drugid) {
        var getData = angularService.getformulaname(drugid);
        getData.then(function (formulaname) {
            var value = formulaname.data.split(",");
            $scope.formulaname = value[0];
            $scope.count = value[1];
        }, function () {
    ...
    $scope.validatequantity = function () {
        if (parseInt($scope.qty) > parseInt($scope.count)) {
            $scope.error = true;
